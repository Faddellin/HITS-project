...
document.addEventListener('DOMContentLoaded', function() {
    ...
    labyrinthForm.addEventListener('submit', function(event) {
        ...
        newTable.id = 'grid';
        const squareDimension = Math.min(document.documentElement.clientWidth, document.documentElement.clientHeight) * 0.8;
        ...
        generateLabyrinthButton.addEventListener('click', function() {
            ...
            for (let x = 0; x < labyrinthSize; x++)
            {
                for (let y = 0; y < labyrinthSize; y++)
                {
                    ...
                    curCell.classList.remove("white");
                    curCell.classList.remove("black");
                    curCell.classList.remove("path");
                    curCell.classList.remove("visited");
                    curCell.classList.remove("start");
                    curCell.classList.remove("goal");
                    if (labyrinth[x][y] === 1)
                    {
